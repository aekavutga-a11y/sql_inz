USE [PLOB]
GO
/***** Object:  StoredProcedure [dbo].[pGetIntefaceInventoryReceive_initial]    Script Date: 10/12/2568 18:01:05 ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO
ALTER PROCEDURE [dbo].[pGetIntefaceInventoryReceive_initial]  
    @Date DATETIME,
...
BEGIN
    ...
    BEGIN TRY
        ...
        WHERE Code = 'INV_RCV' AND BU = 'KKUMD' AND StatusFlag = 'A';

        SELECT @FileName_QSHC = REPLACE(FileName, 'YYYYMMDD', @FileDate)
        FROM InterfaceConfig
        WHERE Code = 'INV_RCV' AND BU = 'QSHC' AND StatusFlag = 'A';
    ...
    ReceiveNo NVARCHAR(MAX),
    ReceiveLineNo NVARCHAR(MAX),
    DeliveryNo NVARCHAR(MAX),
    ReceiveDate NVARCHAR(MAX),
    ItemID NVARCHAR(MAX),
    StorageAreaCode NVARCHAR(MAX),
    ...
    PatientLocation NVARCHAR(MAX),
    ReceivingLocation NVARCHAR(MAX),
    ...
    BatchID NVARCHAR(MAX) 
        ...
        BEGIN
            DECLARE @SQL1 NVARCHAR(MAX) = '
                 INSERT INTO #Interface (
					     [Line]      ,[TransactionGroup]      ,[TransactionID]      ,[BusinessUnitCode]      ,[DistributionType]      ,[ReceiveNo]      ,[ReceiveLineNo]
      ,[DeliveryNo]      ,[ReceiveDate]      ,[ItemID]      ,[StorageAreaCode]      ,[Quantity]      ,[UOM]      ,[Cost]      ,[Vender]      ,[Dept]
      ,[FirstLocation]      ,[PatientLocation]      ,[ReceivingLocation]      ,[EpisodeID]      ,[AvgCost]      ,[BatchID]
      

					)
					EXEC ' + 
					CASE WHEN @Linked1 = '' THEN '' ELSE '[' + @Linked1 + '].' END + 
					'HealthObject.dbo.[pGetCSInterfaceInvenReceive] ''' + @DateFrom + ''', ''' + @DateTo + '''';
      ...
      ,[FirstLocation]      ,[PatientLocation]      ,[ReceivingLocation]      ,[EpisodeID]      ,[AvgCost]      ,[BatchID]
					...
					'HealthObject.dbo.[pGetCSInterfaceInvenReceivePLOB] ''' + @DateFrom + ''', ''' + @DateTo + '''';

            EXEC sp_executesql @SQL2;
        END

        -- Insert into real table
        INSERT INTO Interface_InventoryReceive (
            [FileName],   [Line]      ,[TransactionGroup]      ,[TransactionID]      ,[BusinessUnitCode]      ,[DistributionType]      ,[ReceiveNo]      ,[ReceiveLineNo]
      ,[DeliveryNo]      ,[ReceiveDate]      ,[ItemID]      ,[StorageAreaCode]      ,[Quantity]      ,[UOM]      ,[Cost]      ,[Vender]      ,[Dept]
      ,[FirstLocation]      ,[PatientLocation]      ,[ReceivingLocation]      ,[EpisodeID]      ,[AvgCost]      ,[BatchID]
      ...
      ,[FirstLocation]      ,[PatientLocation]      ,[ReceivingLocation]      ,[EpisodeID]      ,[AvgCost]      ,[BatchID]
        ...
        DECLARE 
            @RowCount1 INT = (SELECT COUNT(*) FROM Interface_InventoryReceive WHERE FileName = @FileName_KKUMD AND StatusFlag = 'A'),
            @RowCount2 INT = (SELECT COUNT(*) FROM Interface_InventoryReceive WHERE FileName = @FileName_QSHC AND StatusFlag = 'A');
        ...
        FROM Interface_InventoryReceive A
